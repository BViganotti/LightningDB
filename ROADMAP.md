...
- [ ] **1.2.2 `edge_types` filtering in `expand`** (`memory.rs:663`): The parameter is accepted but fully ignored. Wire it into the Cypher query as a filter on `r.type IN $edge_types`.
...
- [ ] **2.3.4 Batch materialization**: Replace the current `DataChunk` → `RecordBatch` per-morsel allocation with reusable buffer pools and zero-copy columnar transfer.
- [ ] **2.3.5 Learned prefetch v2**: The Markov-chain prefetch tracker (`prefetch.rs`) is a proof of concept. Extend with:
  - Multi-order Markov chains (currently 1st order only — only tracks immediate next-page)
  - Time-decayed transition weights
  - Automatic confidence threshold tuning

### 2.4 Client ecosystem

- [ ] **2.4.1 Node.js bindings**: Use `napi-rs` to create native Node.js bindings via the C FFI.
- [ ] **2.4.2 Go bindings**: Use `cgo` to wrap the C API. Expose a Go-friendly driver interface.
- [ ] **2.4.3 WebAssembly browser build**: Compile the core engine to WASM via `wasm-pack`. Enable in-browser vector search and graph traversal. Memory-only mode (no disk I/O).
- [ ] **2.4.4 gRPC server**: A standalone gRPC server wrapping Lightning for polyglot microservice access. Bidirectional streaming for CDC events.
- [ ] **2.4.5 LangChain/LlamaIndex integration**: The existing integrations (`langchain.py`, `llama_index.py`) should be tested and documented with real agent examples.
...
- [ ] **2.5.3 Slow query log**: Configurable threshold for logging queries that exceed a duration. Include the query text, plan, and execution stats.
...
- [ ] **3.2.1 Windows CI**: Full test suite passes on Windows.
- [ ] **3.2.2 ARM64 optimization**: NEON SIMD for vector search on ARM (currently only AVX2/SSE for x86).
