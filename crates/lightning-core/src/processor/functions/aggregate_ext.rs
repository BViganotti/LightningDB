...
impl Welford {
    ...
    fn update_from_float(&mut self, x: f64) {
        self.count += 1;
        let d = x - self.mean;
    ...
    fn update_from_array(&mut self, values: &ArrayRef) {
        if let Some(f) = values.as_any().downcast_ref::<arrow::array::Float64Array>() {
            for i in 0..values.len() {
                if !f.is_null(i) { self.update_from_float(f.value(i)); }
            }
        } else if let Some(int_arr) = values.as_any().downcast_ref::<arrow::array::Int64Array>() {
            for i in 0..values.len() {
                if !int_arr.is_null(i) { self.update_from_float(int_arr.value(i) as f64); }
            }
        }
    }

    fn update_from_indices(&mut self, values: &[ArrayRef], row_indices: &[usize]) {
        if values.is_empty() { return; }
        let arr = &values[0];
        if let Some(f) = arr.as_any().downcast_ref::<arrow::array::Float64Array>() {
            for &idx in row_indices {
                if !f.is_null(idx) { self.update_from_float(f.value(idx)); }
            }
        } else if let Some(int_arr) = arr.as_any().downcast_ref::<arrow::array::Int64Array>() {
            for &idx in row_indices {
                if !int_arr.is_null(idx) { self.update_from_float(int_arr.value(idx) as f64); }
            }
        }
    }

    fn merge(&mut self, count: u64, mean: f64, m2: f64) {
        let cc = self.count + count;
        if cc == 0 { return; }
        let d = mean - self.mean;
        self.mean = (self.count as f64 * self.mean + count as f64 * mean) / cc as f64;
        self.m2 = self.m2 + m2 + d * d * self.count as f64 * count as f64 / cc as f64;
        self.count = cc;
    }
}
...
impl StdDevPop {
    pub fn new() -> Self { Self { w: Welford::new() } }
}
...
impl AggregateFunction for StdDevPop {
    ...
    fn update(&mut self, values: &[ArrayRef], row_indices: &[usize]) -> Result<()> {
        self.w.update_from_indices(values, row_indices);
        Ok(())
    }
    fn update_vector(&mut self, values: &ArrayRef) -> Result<()> {
        self.w.update_from_array(values);
        Ok(())
    }
    fn merge(&mut self, other: &dyn AggregateFunction) -> Result<()> {
        if let Some(o) = other.as_any().downcast_ref::<StdDevPop>() {
            self.w.merge(o.w.count, o.w.mean, o.w.m2);
        }
        Ok(())
    }
    fn finalize(&self) -> Result<Value> {
        if self.w.count < 2 { return Ok(Value::Null); }
        Ok(Value::Number((self.w.m2 / self.w.count as f64).sqrt()))
    }
    fn clone_box(&self) -> Box<dyn AggregateFunction> { Box::new(Self { w: Welford { count: self.w.count, mean: self.w.mean, m2: self.w.m2 } }) }
...
impl AggregateFunction for StdDevSamp {
    ...
    fn finalize(&self) -> Result<Value> {
        if self.w.count < 2 { return Ok(Value::Null); }
        Ok(Value::Number((self.w.m2 / (self.w.count - 1) as f64).sqrt()))
    }
    fn clone_box(&self) -> Box<dyn AggregateFunction> { Box::new(Self { w: Welford { count: self.w.count, mean: self.w.mean, m2: self.w.m2 } }) }
    fn as_any(&self) -> &dyn std::any::Any { self }
}
...
impl AggregateFunction for VarPop {
    ...
    fn finalize(&self) -> Result<Value> {
        if self.w.count < 2 { return Ok(Value::Null); }
        Ok(Value::Number(self.w.m2 / self.w.count as f64))
    }
    fn clone_box(&self) -> Box<dyn AggregateFunction> { Box::new(Self { w: Welford { count: self.w.count, mean: self.w.mean, m2: self.w.m2 } }) }
    fn as_any(&self) -> &dyn std::any::Any { self }
}
...
impl AggregateFunction for VarSamp {
    ...
    fn finalize(&self) -> Result<Value> {
        if self.w.count < 2 { return Ok(Value::Null); }
        Ok(Value::Number(self.w.m2 / (self.w.count - 1) as f64))
    }
    fn clone_box(&self) -> Box<dyn AggregateFunction> { Box::new(Self { w: Welford { count: self.w.count, mean: self.w.mean, m2: self.w.m2 } }) }
    fn as_any(&self) -> &dyn std::any::Any { self }
}
